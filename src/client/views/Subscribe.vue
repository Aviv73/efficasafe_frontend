<template>
    <section class="subscribe-page">
        ...
        <div v-if="selectedCoupon || plans" class="cards-container" :class="{ 'justify-center': isCouponZero }">
            ...
            <template v-if="selectedCoupon">
                <div v-for="(plan, idx) in selectedCoupon.plans" :key="idx" class="card"
                    :class="{ 'selected': isSelectedCoupon(idx) }" @click="onSelectCouponPlan(idx)">
                    ...
                    <h3 class="card-title" v-else>Free trial until {{ new Date(selectedCoupon.endTrialDate) |
                        timeFilter('DD/MM/YYYY') }}</h3>
                    <p class="card-price">{{ localCurrency }} <span>{{ getPriceByLocation(plan) }}</span> /mo</p>
                    <p class="card-billing-txt" v-if="!selectedCoupon.endTrialDate">{{ billingTxt(plan) }}</p>
                    <button class="card-btn"
                        :class="{ 'margin-top': selectedCoupon.endTrialDate }">{{ couponSelectBtnTxt((idx)) }}</button>
                </div>
            </template>
        ...
        <div class="coupon-input-container">
            <input v-model="couponInput" type="text" placeholder="Coupon Code" @input="onInputCoupon" class="coupon-input"
                :class="{ 'invalid-input': isCouponInvalid }" />
            <button @click="onSearchCoupon" ref="submitCoupon" class="coupon-input-btn"
                :class="{ 'invalid-btn': isCouponInvalid }">{{ couponBtnTxt }}</button>
        </div>
        ...
        <div class="benefits-container">
            ...
            <div class="benefit">
                ...
                <p><span>Increase patient safety</span> via access to up to date Drug-Drug and Herb-Supplement-Drug
                    interaction data</p>
            </div>
            <div class="benefit">
                <img src="@/client/assets/imgs/logo-symbol-sm.png" alt="">
                <p><span>Make accurate clinical decisions</span> by leveraging extensive evidence-based information on
                    clinical efficacy and patient safety</p>
            </div>
            <div class="benefit">
                <img src="@/client/assets/imgs/logo-symbol-sm.png" alt="">
            ...
            <div class="benefit">
                <img src="@/client/assets/imgs/logo-symbol-sm.png" alt="">
                <p><span>Gain</span> valuable insights on which clinical and laboratory parameters to <span>monitor</span>
                    per individual patient profiles/interactions</p>
            </div>
            <div class="benefit">
                <img src="@/client/assets/imgs/logo-symbol-sm.png" alt="">
                <p><span>Optimize integrative treatment</span> via vital information on both negative and synergistic
                    Herb-Supplement-Drug interactions</p>
            </div>
            <div class="benefit">
                <img src="@/client/assets/imgs/logo-symbol-sm.png" alt="">
                <p><span>Save patients' pharmacological profiles.</span> Manage, save, and track updated patient
                    drug-herb-supplement profiles and records quickly and easily</p>
            </div>
            <div class="benefit">
                <img src="@/client/assets/imgs/logo-symbol-sm.png" alt="">
                <p><span>Empower patients</span> with reliable, referenced information to help them better understand their
                    medications and/or supplements while identifying any potentially dangerous interactions before they
                    occur</p>
            </div>
        </div>
        <div class="payment-disclaimer">
            <p>Your subscription will automatically renew every month if you select the Monthly Plan, every 12 months if you
                select the Annual Plan, or every twenty-four months if you select the Biennial Plan. You will be charged the
                price listed above for the plan you select, plus applicable taxes, on each renewal until you cancel in your
                account settings. If you cancel, previous charges will not be refunded, but you may continue to use the
                service until the end of the term you paid for.</p>
            <p>By clicking the "Continue to payment" button above, you are agreeing to our <a
                    href="/terms-and-conditions">Terms And Conditions</a> and acknowledge that you have read our <a
                    href="/privacy-statement"> Privacy Policy</a>.</p>
        </div>
        <modal-wrap :isActive="isGroupSubModal" @close-modal="closeGroupSubModal">
            <div class="group-sub-modal-container">
                ...
                <template v-else>
                    <img class="group-sub-modal-container-logo" src="@/client/assets/imgs/flat-logo.webp" alt="Logo" />
                    <form @submit.prevent="onSubmit">
                        <h3 v-if="!isShowError">Please fill in the details</h3>
                        <h3 v-else style="color: red">Please fill in all the fields</h3>
                        <input class="reg-input" type="text" placeholder="Institution / Company Name"
                            v-model="groupSubData.companyName" @change="isShowError = false" />
                        <input type="text" class="reg-input" placeholder="Type (e.g. Hospital, Clinic, Etc)"
                            v-model="groupSubData.type" @change="isShowError = false" />
                        <input type="text" class="reg-input" placeholder="Number Of licenses needed"
                            v-model="groupSubData.numberOfEmployees" @change="isShowError = false" />
                        <input type="text" class="reg-input" placeholder="Contact full Name"
                            v-model="groupSubData.contactName" @change="isShowError = false" />
                        <input type="text" class="reg-input" placeholder="Email" v-model="groupSubData.contactEmail"
                            @change="isShowError = false" />
                        <div class="form-input last-input-form">
                            <label for="">
                                <input type="text" placeholder="Phone" v-model="groupSubData.phoneNumber" ref="phoneInput"
                                    @countrychange="setPhoneDialCode" @change="isShowError = false" />
