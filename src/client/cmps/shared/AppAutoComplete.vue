<template>
  <form @submit.prevent="" class="autocomplete">
    <div class="autocomplete-field">
      <input v-model="search" ref="searchInput" type="text" class="primary" :placeholder="placeholder" @keyup.down="shiftFocus(0)" v-debounce="getResults" />
      ...
      >
        <li tabindex="0" v-for="(result, idx) in results" :ref="`result${idx}`" :key="idx" class="autocomplete-results-result" @click="emitSelection(result)" @keyup.enter="emitSelection(result)" @keydown.up.prevent="shiftFocus(idx - 1)" @keydown.down.prevent="shiftFocus(idx + 1)">
          {{ result.txt }}
        </li>
      </ul>
    </div>
