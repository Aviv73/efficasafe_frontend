<template>
  <section class="vertical-list">
    <div v-if="lists">
      <header class="vertical-list-header">
        <div class="flex-center">
          <interaction-icon class="interaction-icon" :color="getInteractionColor('avoid coadministration')" />
          Avoid
          <span class="count" v-if="lists.reds.total"> ({{ lists.reds.total }}) </span>
        </div>
        <div class="flex-center">
          <interaction-icon class="interaction-icon" :color="getInteractionColor('caution should be taken')" />
          Require caution
          <span class="count" v-if="lists.yellows.total"> ({{ lists.yellows.total }}) </span>
        </div>
        <div class="flex-center">
          <interaction-icon class="interaction-icon" :color="getInteractionColor('coadministration is possible')" />
          Synergistic
          <span class="count" v-if="lists.greens.total"> ({{ lists.greens.total }}) </span>
        </div>
      </header>
      <main class="vertical-list-lists">
        <ul>
          <li class="vertical-list-lists-row" :class="{ longest: isLongestList(lists.reds) }" v-for="interaction in lists.reds[interactions]" :key="interaction._id">
            <tooltip v-if="interaction.isLead" bottom stand-alone :offsetY="16" :offsetX="2">
              <template #content>
                <p class="vertical-list-lists-row-notf-msg">There are multiple interactions for these materials but they are visible only in the default view</p>
              </template>
              <span class="vertical-list-lists-row-notf">
                <bell-ring-icon :size="18" title="" />
              </span>
            </tooltip>
            <tooltip :hidden="getPreviewWrapEl(interaction) === 'router-link'" left>
              <template #content>
                <span class="label-msg"> This interaction can not be viewed because it is an interaction with a group of drugs. Please use horizontal view to see the interaction details </span>
              </template>
              <component :is="getPreviewWrapEl(interaction)" :to="getInteractionLink(interaction)">
                <interaction-capsules :title="interaction.recommendation" :name="getInteractionName(interaction)" :color="getInteractionColor('avoid coadministration')" :vInteractionCount="0" :showDraftName="false" localize dense />
              </component>
            </tooltip>
          </li>
        </ul>
        <ul>
          ...
          >
            <tooltip v-if="interaction.isLead" bottom stand-alone :offsetY="16" :offsetX="2">
              ...
              <span v-if="interaction.isLead" class="vertical-list-lists-row-notf">
                <bell-ring-icon :size="18" title="" />
              </span>
            </tooltip>
            <tooltip :hidden="getPreviewWrapEl(interaction) === 'router-link'" right>
              <template #content>
                <span class="label-msg"> This interaction can not be viewed because it is an interaction with a group of drugs. Please use horizontal view to see the interaction details </span>
              </template>
              <component :is="getPreviewWrapEl(interaction)" :to="getInteractionLink(interaction)">
                <interaction-capsules :title="interaction.recommendation" :name="getInteractionName(interaction)" :color="getInteractionColor('caution should be taken')" :vInteractionCount="0" :showDraftName="false" localize dense />
              </component>
            </tooltip>
          </li>
        </ul>
        <ul>
          <li class="vertical-list-lists-row" :class="{ longest: isLongestList(lists.greens) }" v-for="interaction in lists.greens[interactions]" :key="interaction._id">
            <tooltip v-if="interaction.isLead" bottom stand-alone :offsetY="16" :offsetX="2">
              <template #content>
                <p class="vertical-list-lists-row-notf-msg">There are multiple interactions for these materials but they are visible only in the default view</p>
              </template>
            ...
            <tooltip :hidden="getPreviewWrapEl(interaction) === 'router-link'" right>
              ...
              <component :is="getPreviewWrapEl(interaction)" :to="getInteractionLink(interaction)">
                <interaction-capsules :title="interaction.recommendation" :name="getInteractionName(interaction)" :color="getInteractionColor('coadministration is possible')" :vInteractionCount="0" :showDraftName="false" localize dense />
              </component>
            </tooltip>
          </li>
        </ul>
      </main>
      <list-pagination class="list-pagination flex-center" v-model.number="page" v-if="maxPageCount > 1" :pageCount="maxPageCount" :disabled="isLoading">
        <template #first-btn>
    ...
    <div v-else-if="$route.name === 'Boosters' || $route.name === 'Monitor'">
      <p class="vertical-list-msg">{{ $route.name === 'Boosters' ? 'Optimizers are' : 'Monitor is' }} only available in default view.</p>
    </div>
  </section>
</template>
